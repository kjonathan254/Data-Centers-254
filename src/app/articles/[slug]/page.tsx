...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      authors: ["Kevin Jonathan Otieno"],
      section: article.cluster,
      tags: clusterKeywords[article.cluster]?.slice(0, 5),
    },
    twitter: {
...
export default async function ArticlePage({ params }: PageProps) {
  ...
  // Get related articles (same cluster, excluding current)
  ...
  const relatedJson = JSON.parse(JSON.stringify(related));
  ...
  const articleLd = {
    ...
    url: `https://datacentre254.com/articles/${article.slug}`,
    datePublished: article.createdAt.toISOString(),
    dateModified: article.updatedAt.toISOString(),
    author: {
      "@type": "Person",
      name: "Kevin Jonathan Otieno",
      url: "https://datacentre254.com/about",
    ...
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `https://datacentre254.com/articles/${article.slug}`,
    },
    articleSection: article.cluster,
  };

  // BreadcrumbList JSON-LD
  const clusterSlug = article.cluster.toLowerCase();
  const breadcrumbLd = {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    itemListElement: [
      {
        ...
        position: 1,
        name: "Home",
        item: "https://datacentre254.com",
      },
      {
        "@type": "ListItem",
        position: 2,
        name: article.cluster,
        item: `https://datacentre254.com/${clusterSlug}`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: article.title,
        item: `https://datacentre254.com/articles/${article.slug}`,
      },
    ],
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        ...
        />
        <ArticlePageClient article={articleJson} related={relatedJson} cluster={article.cluster} />
      </main>
      <Footer />
    </div>
  );
